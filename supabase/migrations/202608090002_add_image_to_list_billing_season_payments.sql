-- Correctif du même jour (2026-08-09), en migration de suivi — jamais en
-- modifiant 202608090001 déjà appliquée.
--
-- Oubli identifié en construisant la couche TypeScript : pour un versement
-- UNIQUE (chèque ou virement non partagé), `cheque-register.ts`
-- (`collecterOperationsBancaires`) affiche le justificatif attaché à sa
-- propre opération dédiée — c'est le seul moyen de savoir s'il en existe un
-- pour proposer d'en ajouter un (R-35/R-36). Pour un versement PARTAGÉ, ce
-- même justificatif vient déjà de `operations` (chargé séparément, inchangé
-- par ce lot) : les colonnes ajoutées ici sont donc lues seulement pour les
-- lignes uniques, mais renvoyées pour toutes par simplicité de jointure.
--
-- `CREATE OR REPLACE` ne permet pas de changer la forme d'une fonction
-- TABLE : DROP puis CREATE, sans risque ici puisqu'aucun code déployé ne
-- consomme encore cette RPC.
...
comment on function public.list_billing_season_payments(
  ...
  'd''opération séparées, et le justificatif actif de l''opération (utile '
  'pour un versement unique — pour un versement partagé, le justificatif '
  'vient déjà de la liste des opérations réutilisables, chargée séparément) '
  '— remplace le chargement de tous les reçus complets pour Paiements, '
  'Journal financier et Suivi journalier.';
