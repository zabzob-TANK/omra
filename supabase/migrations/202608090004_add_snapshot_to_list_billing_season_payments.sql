...
-- modifiant 202608090001/202608090002 déjà appliquées.
--
-- Oubli identifié en construisant `service.ts` (`journalFinancier`) :
-- chaque ligne du Journal financier est construite depuis l'instantané figé
-- du versement (R-14, R-22 — client, programme, convenu, restant après CE
-- versement précis, statut après), jamais recalculé depuis l'état actuel du
-- reçu. `list_billing_season_payments` ne portait pas ces colonnes.
--
-- `DROP` puis `CREATE`, comme 202608090002 : aucun code déployé ne consomme
-- encore cette RPC.
...
comment on function public.list_billing_season_payments(
  ...
  'd''opération séparées, le justificatif actif de l''opération, et '
  'l''instantané figé du versement (R-14, R-22) — remplace le chargement de '
  'tous les reçus complets pour Paiements, Journal financier et Suivi '
  'journalier.';
