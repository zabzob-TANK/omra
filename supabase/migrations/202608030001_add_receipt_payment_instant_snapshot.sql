-- Ajoute l'instantané figé exigé par reprise.md §5.7 : chaque versement doit
-- conserver l'état du reçu au moment où il a été enregistré (client, hôtel,
-- chambre, vol, programme, convenu, rabatteur, restant après, soldé après).
--
-- Le schéma existant ne portait aucune de ces colonnes : seules les
-- inscriptions (traveler_registrations) ont des *_snapshot, figées une fois
-- à l'inscription, jamais par versement. Cette migration comble cet écart
-- pendant que la table est vide — la seule fenêtre où l'ajout de colonnes
-- NOT NULL sans valeur par défaut est gratuit.
--
-- create_billing_receipt_with_first_payment et add_billing_receipt_payment
-- sont redéfinies pour renseigner ces colonnes à l'écriture. Elles ne sont
-- jamais recalculées ensuite. get_billing_receipt_details est redéfinie pour
-- les exposer telles quelles.

alter table public.receipt_payments
  add column payment_snapshot_client_name text,
  add column payment_snapshot_hotel_name text,
  add column payment_snapshot_room_label text,
  add column payment_snapshot_flight_label text,
  add column payment_snapshot_program_label text,
  add column payment_snapshot_agreed_amount_dh integer,
  add column payment_snapshot_rabatteur_name text,
  add column payment_snapshot_remaining_after_dh integer,
  add column payment_snapshot_settled_after boolean;

alter table public.receipt_payments
  alter column payment_snapshot_client_name set not null,
  alter column payment_snapshot_hotel_name set not null,
  alter column payment_snapshot_room_label set not null,
  alter column payment_snapshot_flight_label set not null,
  alter column payment_snapshot_program_label set not null,
  alter column payment_snapshot_agreed_amount_dh set not null,
  alter column payment_snapshot_remaining_after_dh set not null,
  alter column payment_snapshot_settled_after set not null;

-- payment_snapshot_rabatteur_name reste nullable : le rabatteur lui-même est
-- optionnel sur une inscription (traveler_registrations.rabatteur_id est
-- nullable), donc son nom instantané l'est aussi.

alter table public.receipt_payments
  add constraint receipt_payments_snapshot_client_name_check check (
    btrim(payment_snapshot_client_name) <> ''
  ),
  add constraint receipt_payments_snapshot_hotel_name_check check (
    btrim(payment_snapshot_hotel_name) <> ''
  ),
  add constraint receipt_payments_snapshot_room_label_check check (
    btrim(payment_snapshot_room_label) <> ''
  ),
  add constraint receipt_payments_snapshot_flight_label_check check (
    btrim(payment_snapshot_flight_label) <> ''
  ),
  add constraint receipt_payments_snapshot_program_label_check check (
    btrim(payment_snapshot_program_label) <> ''
  ),
  add constraint receipt_payments_snapshot_agreed_amount_check check (
    payment_snapshot_agreed_amount_dh > 0
  ),
  add constraint receipt_payments_snapshot_remaining_after_check check (
    payment_snapshot_remaining_after_dh >= 0
  ),
  add constraint receipt_payments_snapshot_settled_after_consistency_check check (
    payment_snapshot_settled_after = (payment_snapshot_remaining_after_dh <= 0)
  );
...
comment on function public.create_billing_receipt_with_first_payment(
  ...
  'Creates one seasonal receipt, its mandatory first payment, its payment operation and allocation atomically. Actor fields come only from resolve_facturation_actor(). The first payment stores a frozen instant snapshot (client, hotel, room, flight, program, agreed amount, rabatteur, remaining after, settled after), computed once here and never recalculated. Stable history actions are billing_receipt.created, billing_receipt.first_payment_added and payment_operation.over_allocation_confirmed.';
...
begin
  ...
  v_payment_snapshot_program_label :=
    v_hotel_name_snapshot || ' / غرفة ' || v_room_label_snapshot || ' / ' ||
    v_flight_label_snapshot;
  v_settled_after := v_receipt_remaining_after_dh <= 0;
...
comment on function public.add_billing_receipt_payment(
  ...
  'Adds one server-numbered payment from 2 through 6 to an active receipt. Actor fields come only from resolve_facturation_actor(). Each payment stores a frozen instant snapshot (client, hotel, room, flight, program, agreed amount, rabatteur, remaining after, settled after), computed once here and never recalculated. Stable history actions are billing_receipt.payment_added, payment_operation.created, payment_operation.reused and payment_operation.over_allocation_confirmed.';
