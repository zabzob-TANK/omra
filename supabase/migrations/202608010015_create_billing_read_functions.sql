...
comment on function public.list_billing_receipts(
  uuid,
  timestamptz,
  timestamptz,
  text,
  ...
  'Read-only paginated receipt list. Financial status, payment totals, modification counters, commercial anomalies and missing cheque evidence are calculated from source rows. Technical dossier references are returned as NULL. No print data is returned because no reliable print field or event exists.';

create function public.get_billing_receipt_details(
  p_receipt_id uuid
)
returns jsonb
language plpgsql
...
comment on function public.get_billing_receipt_details(uuid) is
  'Read-only receipt detail built from historical registration snapshots, payments, allocations, operations, active evidence, cancellation and precisely related history. Deleted evidence and Auth identifiers are omitted; private bucket/path metadata is returned without any URL or token. Printing is omitted because the current schema has no reliable print field or event.';

create function public.list_billing_anomalies(
  p_season_id uuid default null,
  p_appeared_from timestamptz default null,
  p_appeared_to timestamptz default null,
...
begin
  ...
  if p_limit is null or p_limit <= 0 or p_limit > 200 then
    raise exception 'Billing anomaly page limit must be between 1 and 200';
  end if;

  if p_offset is null or p_offset < 0 then
    raise exception 'Billing anomaly page offset must be non-negative';
  end if;

  if p_appeared_from is not null and p_appeared_to is not null and
     p_appeared_from > p_appeared_to then
    raise exception 'Billing anomaly date range is invalid';
  end if;

  if p_anomaly_type is not null and p_anomaly_type not in (
    'amount_due',
    'overpayment',
    'cheque_missing_supporting_image'
  ) then
    raise exception 'Billing anomaly type is invalid';
  end if;
  ...
  financial_anomalies as (
    select
      'receipt:' || receipt.id::text || ':' ||
        case
          when registration.agreed_amount_dh::bigint >
               coalesce(payment.total_paid_dh, 0)::bigint
            then 'amount_due'
          else 'overpayment'
        end as anomaly_id,
      case
      ...
      case
        when registration.agreed_amount_dh::bigint >
             coalesce(payment.total_paid_dh, 0)::bigint
          then registration.agreed_amount_dh::bigint -
            coalesce(payment.total_paid_dh, 0)::bigint
        else coalesce(payment.total_paid_dh, 0)::bigint -
          registration.agreed_amount_dh::bigint
      end as amount_dh,
      receipt.id as receipt_id,
      null::uuid as operation_id,
      array[receipt.id]::uuid[] as related_receipt_ids,
      array[receipt.season_id]::uuid[] as related_season_ids,
      commercial.occurred_at as appeared_at,
      commercial.occurred_at as last_changed_at,
      case
        when registration.agreed_amount_dh::bigint >
             coalesce(payment.total_paid_dh, 0)::bigint
          then 'amount_due_after_commercial_change'
        else 'overpayment_after_commercial_change'
      end as message_code,
      receipt.lifecycle_status
    from public.billing_receipts as receipt
    join public.traveler_registrations as registration
      on registration.id = receipt.registration_id
    join latest_commercial_modification as commercial
      on commercial.receipt_id = receipt.id
    left join payment_summary as payment
      on payment.receipt_id = receipt.id
    where commercial.after_data @>
      '{"has_financial_anomaly": true}'::jsonb
      and registration.agreed_amount_dh::bigint <>
        coalesce(payment.total_paid_dh, 0)::bigint
  ),
  cheque_operation_groups as (
    select
      operation.id as operation_id,
      pg_catalog.array_agg(
        distinct receipt.id order by receipt.id
      ) as related_receipt_ids,
      pg_catalog.array_agg(
        distinct receipt.season_id order by receipt.season_id
      ) as related_season_ids,
      pg_catalog.min(allocation.allocated_at) as first_allocated_at,
      pg_catalog.max(image.deleted_at) as last_image_deleted_at
    from public.payment_operations as operation
    join public.payment_allocations as allocation
      on allocation.payment_operation_id = operation.id
    join public.receipt_payments as payment
      on payment.id = allocation.receipt_payment_id
    join public.billing_receipts as receipt
      on receipt.id = payment.receipt_id
    left join public.payment_supporting_images as image
      on image.payment_operation_id = operation.id
    where operation.payment_mode = 'cheque'
      and not exists (
        select 1
        from public.payment_supporting_images as active_image
        where active_image.payment_operation_id = operation.id
          and active_image.deleted_at is null
      )
  ...
  cheque_anomalies as (
    select
      'operation:' || cheque.operation_id::text ||
        ':cheque_missing_supporting_image' as anomaly_id,
      'cheque_missing_supporting_image'::text as anomaly_type,
      null::bigint as amount_dh,
      case
        when pg_catalog.cardinality(cheque.related_receipt_ids) = 1
          then cheque.related_receipt_ids[1]
        else null::uuid
      end as receipt_id,
      cheque.operation_id,
      cheque.related_receipt_ids,
      cheque.related_season_ids,
      coalesce(
        cheque.last_image_deleted_at,
        cheque.first_allocated_at
      ) as appeared_at,
      coalesce(
        cheque.last_image_deleted_at,
        cheque.first_allocated_at
      ) as last_changed_at,
      'cheque_missing_supporting_image'::text as message_code,
      case
        when pg_catalog.cardinality(cheque.related_receipt_ids) = 1 then (
          select receipt.lifecycle_status
          from public.billing_receipts as receipt
          where receipt.id = cheque.related_receipt_ids[1]
        )
        else null::text
      end as lifecycle_status
    from cheque_operation_groups as cheque
  ),
  all_anomalies as (
    select anomaly.* from financial_anomalies as anomaly
    union all
    select anomaly.* from cheque_anomalies as anomaly
  ),
  filtered_anomalies as (
    select anomaly.*
    from all_anomalies as anomaly
    where (
        p_season_id is null or
        p_season_id = any(anomaly.related_season_ids)
      )
      and (
        p_appeared_from is null or
        anomaly.appeared_at >= p_appeared_from
      )
      and (
        p_appeared_to is null or
        anomaly.appeared_at <= p_appeared_to
      )
      and (
        p_anomaly_type is null or
        anomaly.anomaly_type = p_anomaly_type
      )
      and (
        p_receipt_id is null or
        p_receipt_id = any(anomaly.related_receipt_ids)
      )
      and (
        p_operation_id is null or
        anomaly.operation_id = p_operation_id
      )
  )
  select
    anomaly.anomaly_id,
    ...
    anomaly.amount_dh,
    anomaly.receipt_id,
    anomaly.operation_id,
    anomaly.related_receipt_ids,
    anomaly.related_season_ids,
    anomaly.appeared_at,
    anomaly.last_changed_at,
    anomaly.message_code,
    anomaly.lifecycle_status,
    pg_catalog.count(*) over () as total_rows
  from filtered_anomalies as anomaly
  order by anomaly.last_changed_at desc, anomaly.anomaly_id
  limit p_limit
  offset p_offset;
end;
$$;

comment on function public.list_billing_anomalies(
  uuid,
  timestamptz,
  timestamptz,
  text,
  ...
  'Read-only active anomaly list. Financial anomalies require the latest commercial change to have reported an anomaly and are recalculated from current amounts. Missing evidence is reported once per linked cheque operation, never for cash or transfer. receipt_not_printed is omitted because no reliable print data exists.';
...
revoke execute on function public.get_billing_receipt_details(uuid)
  from public, anon;

grant execute on function public.get_billing_receipt_details(uuid)
  to authenticated;

revoke execute on function public.list_billing_anomalies(
  uuid,
  timestamptz,
  timestamptz,
  text,
...
grant execute on function public.list_billing_anomalies(
  uuid,
  timestamptz,
  timestamptz,
  text,
